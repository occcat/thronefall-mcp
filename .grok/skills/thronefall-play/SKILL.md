...
- **守得住才扩张。** 护不住的田、港、矿先别造。同一类收入建筑按「今晚会不会被拆」分别判断，不要把三座港或三座矿当一件事。房子通常最安全（靠城堡、两夜回本）。磨坊/港口一级往往不值，升级后才是大头。矿前高后低，能摸到且今晚盖得住就早开。
...
- **够打也要把当前能买、回得来的收入扫完再停。** 「战力够了」不是停手理由。漏掉 1 金田/房等于后面每夜少收。

## 开夜前把钱花完

买一轮 → `GET /state` 看 `balance` 和还能升级的格子 → 再买 → 再查，至少两轮。**默认花完。** 只有写得出战略理由才许留金（Interest、明天城堡档、明确的下一夜大件）。「好像够了」不是理由。没有 Interest 时剩 10+ 就继续扫田/房/该口塔，直到买不起下一档有用的东西。
