...
- **小兵不要固守。** `/units/command`、`/units/send-to-spawn`、HTTP `/units/deploy` 的 API 默认 `hold=true`；本局一律显式传 `"hold": false`。不要 `/units/hold`，不要 `/units/follow` 把整队绑到国王。
...
HTTP 有、MCP 未单独包的：`POST /units/deploy`（`picks` / `target` / `hold` / `spacing=2`）、`POST /slots/{id}/build`（与 upgrade 同义）、`POST /slots/{id}/choice`、`POST /units/groups`、`GET /state/training`、`GET /state/slots|units|enemies|spawns|loadout`。
...
- 按种类/数量瞬移：HTTP `POST /units/deploy`（`picks` + `target` + `"hold": false` + `spacing` 默认 2）。
